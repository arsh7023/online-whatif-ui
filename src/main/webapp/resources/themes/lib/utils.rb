  ...
  module SassExtensions
    module Functions
      module Utils
        ...
        def theme_image(theme, path, without_url = false, relative = true)
          path = path.value
          theme = theme.value
          ...
          if relative
            ...
            # fixed for aura
            images_path = File.join('..', 'themes', 'images', theme)
        ...
        def theme_image_exists(path)
          ...
          # fixed for aura
          # path would be '../themes/images/...'
          where_to_look = File.join($resource_path, 'themes', path.value)
