...
@Component("uploader")
public class Uploader extends AbstractWifServlet {
  ...
  public void doPost(final HttpServletRequest request,
    ...
    try {
      ...
      if (inputStream != null && entityRequest) {

        final byte[] entityBytes = IOUtils.toByteArray(inputStream);
        final Enumeration<String> contentTypeHeaders = request
            .getHeaders(HTTP.CONTENT_TYPE);
        String entityEncoding = "multipart/form-data";
        if (contentTypeHeaders.hasMoreElements()) {
          entityEncoding = contentTypeHeaders.nextElement();
          if (contentTypeHeaders.hasMoreElements()) {
            throw new ServletException(
                "HTTP request appears to have more than one Content-Type header,");
          }
        }
